...
| --- | --- | --- | --- | --- |
| R-001 | Multiple AI entry points can drift into separate runtimes | Critical | Agent, chief-of-staff, recommendations, module helpers, recorder processing, and universal input all exist | Define one authoritative Jarvis orchestration path; keep capability adapters behind it |
| R-002 | AI can appear successful while operating in stub mode | Critical | Provider and transcription services contain deterministic `[STUB]` fallbacks | Replace synthetic success with typed `capability_unavailable` results and visible configuration requirements |
| R-003 | No universal governed Tool Gateway | Critical | AI services call repositories/services through feature-specific code | Register every executable operation with schema, risk, permission, approval, timeout, idempotency, and verification contracts |
| R-004 | Approval semantics are feature-specific rather than universal | Critical | Action drafts are approval-oriented, but there is no exact-operation approval object for all writes | Build immutable approval requests bound to owner, tool, version, argument hash, expiry, and single use |
| R-005 | Render is acting as first-line build validation | High | Recent production deploy failed on a TypeScript error; GitHub showed no checks | Add mandatory CI for install, typecheck, lint, tests, build, migration checks, and security contracts |
...
| R-009 | Event history is distributed and may not form one operational ledger | High | System events, agent events, action transitions, and provider activity exist in separate shapes | Normalize trace IDs and append-only operational records across runs, tools, approvals, jobs, modules, and providers |
...
| R-013 | Module failure can degrade global decision quality silently | Medium | Safe fanout now avoids total failure, but degraded context quality needs explicit surfacing | Include degraded-module evidence in Jarvis plans and prevent high-confidence recommendations when critical modules are unavailable |
...
| R-017 | UI complexity exposes infrastructure instead of outcomes | High | Current page-centric experience surfaces many cards, modules, statuses, and provider details | Replace primary UX with Today, Jarvis, Capture, Work, History; move system control to admin settings |
...
| R-019 | External side effects may lack idempotency and receipts | Critical | Full external-write inventory not complete | Require idempotency keys, exact approval, postcondition verification, and operation receipts for every external write |
| R-020 | No production SLO/error budget currently governs Jarvis behavior | Medium | Performance and reliability targets are documented but not enforced | Define measured SLOs for acknowledgement, tool execution, job completion, availability, duplicate writes, and unauthorized writes |
...
4. Define Tool Gateway contracts.
5. Define exact-operation Approval Engine contracts.
6. Add trace IDs and operation receipts.
7. Move recorder processing to durable jobs.
8. Build the first end-to-end Jarvis read and governed-write vertical slice.
